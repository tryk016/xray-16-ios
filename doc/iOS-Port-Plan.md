...
- **Phase 4 — GLES 3.0 / ANGLE-on-Metal renderer + shaders + textures**
  ...
  - 🟡 4.4 Rewrite the shared shims common.h and common_samplers.h for GLSL ES 3.00 — 4.4a `#ifdef GL_ES` precision prelude + default option macros (SUN/SSR/MSAA) in common.h; 4.4b float-literal shims (`clip`, lmodel) done; **"family C" int/float grind (commits labelled Slice 4.8): GLSL ES 3.00 has no implicit int→float — batch-fixed literals + `float*int2` across the shared headers (gather/shadow/sload/hmodel/lmodel) + moved `VARYING()` into shared/common.h + guarded `gl_ClipDistance`. Now iterated OFFLINE via a LOCAL glslangValidator (`tools/glslang/`) — gate 117→230/286 (80%).** Remaining: ~56 scattered per-shader int/float + fxaa/ssao undefined-macro defaults + ATOC `_main`; sampler-precision link edge cases still to verify on-device
