...
- **Phase 4 — GLES 3.0 / ANGLE-on-Metal renderer + shaders + textures**
  ...
  - 🟡 4.4 Rewrite the shared shims common.h and common_samplers.h for GLSL ES 3.00 — 4.4a `#ifdef GL_ES` precision prelude + default option macros (SUN/SSR/MSAA) in common.h; 4.4b float-literal shims (`clip`, lmodel) done; **"family C" int/float grind (commits labelled Slice 4.8): GLSL ES 3.00 has no implicit int→float — batch-fixed literals + `float*int2` across the shared headers (gather/shadow/sload/hmodel/lmodel) + moved `VARYING()` into shared/common.h + guarded `gl_ClipDistance`. Now iterated OFFLINE via a LOCAL glslangValidator (`tools/glslang/`) — gate 117→265/279 (95%).** Remaining 14 = one issue ("family D": model-VS skinning vertex-format, `I.N=v_model_N` float4-attribute vs float3-struct — variant-aware fix + on-device verify); sampler-precision link edge cases still to verify on-device
  - 🟡 4.5 Regenerate the 81 iostructs/ headers for ES — 4.5a `gl_PerVertex` wrapped `#ifndef GL_ES`; 4.5b `VARYING()` macro strips `layout(location)` off vs↔fs varyings under ES; **4.5c** guard `gl_FragCoord`/`gl_SampleID` redeclaration `#ifndef GL_ES` (32 across 24 files — first real on-device ES compile error) + gate now models `GBUFFER_OPTIMIZATION`. Remaining: A2 name-matched varyings (link-time), layout(location) fragment outputs
