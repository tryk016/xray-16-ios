...
- **Phase 4 — GLES 3.0 / ANGLE-on-Metal renderer + shaders + textures**
  ...
  - 🟡 4.4 Rewrite the shared shims common.h and common_samplers.h for GLSL ES 3.00 — 4.4a `#ifdef GL_ES` precision prelude + default option macros (SUN/SSR/MSAA) in common.h; 4.4b float-literal shims (`clip`, lmodel) done; sampler-precision link edge cases still to verify on-device
